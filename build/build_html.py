...
# The social-card image (og:image / twitter:image). Referenced by ABSOLUTE URL, exactly like the
# canonical link: it is a string pointer in the markup, never a fetch, so it costs the offline
# file:// copy nothing and does not break the zero-external-asset property. Only a link-preview
# crawler resolving the HOSTED page ever requests it; a file:// reader carries an inert pointer.
# The PNG is a committed asset generated by tools/make_og_image.mjs — build_html.py does not run it.
OG_IMAGE_FILENAME = "og-image.png"
OG_IMAGE_URL = SITE_URL + OG_IMAGE_FILENAME
...
def main():
    ...
    # Alt text describes the card itself (name + tagline), which is exactly what the countless card
    # shows — no counts, so nothing here goes stale.
    og_image_alt = f'{meta["brand"]["title"]} — {meta["brand"]["tagline"]}'
    for token, value in (("__BRAND_TITLE__", meta["brand"]["title"]),
                         ...
                         ("__OG_IMAGE_URL__", OG_IMAGE_URL),
                         ("__OG_IMAGE_ALT__", og_image_alt)):
...
def write_index(meta):
    ...
    doc = INDEX_TEMPLATE.format(
        ...
        image=html.escape(OG_IMAGE_URL, quote=True),
        image_alt=html.escape(f'{meta["brand"]["title"]} — {meta["brand"]["tagline"]}', quote=True),
